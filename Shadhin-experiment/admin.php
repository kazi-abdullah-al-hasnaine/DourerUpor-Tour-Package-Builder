...
require_once('DesignPatterns/PackageObserver.php'); // Include the Observer pattern file

// Initialize the subject (observable)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['package_id']) && isset($_POST['action'])) {
        ...
        if ($action === 'approve') {
            if ($package->approve()) {
                // Notify followers about approval
            ...
            if ($package->reject($feedback)) {
                // Notify followers about rejection
                $packageInfo = $package->getPackageInfo($packageId);
            ...
            // Notify followers about pending status
            $packageInfo = $package->getPackageInfo($packageId);
...
// Base query
$query = "
    SELECT p.package_id, p.package_name, p.publish_time, p.status, 
           u.name as creator_name, p.details, p.image
    FROM packages p
